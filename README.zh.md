...
- 启动台**装完就有那九张卡**：名称、说明、封面与两个类目随包提供，装完即与截图里的启动台一致。
...
员工信息管理、知识库阅读、项目管理、应用开发、小红书、UI 组件库，以及「注册一个新应用」这个入口，
各自都是独立的应用／插件。点开一张未接入的卡，提示是「应用已注册但未接入」。本包提供的是启动台
这个容器、卡片的呈现方式，以及它装完时的这份清单；你的启动台里能真正打开什么，取决于你装了什么。
